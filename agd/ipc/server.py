...
class IPCServer:
    ...
    def start(self):
        threading.Thread(target=self._push_loop,  daemon=True, name='agd-push').start()
        threading.Thread(target=self._cmd_server,  daemon=True, name='agd-cmd').start()
    ...
    def _push_loop(self):
        ...
        srv.bind(PUSH_SOCK)
        srv.listen(8)
        log.info("AGD push socket: %s", PUSH_SOCK)

        threading.Thread(target=self._accept_subs, args=(srv,),
                         daemon=True, name='agd-push-accept').start()
    ...
    def _cmd_server(self):
        ...
        srv.bind(CMD_SOCK)
        srv.listen(8)
        log.info("AGD cmd socket: %s", CMD_SOCK)
