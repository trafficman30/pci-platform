# rtig/ipc/server.py
# IPC server for pci.rtig — push socket + command socket.
# Follows the same pattern as pci/ug405/ipc/server.py.
#
# Push socket  /tmp/pci.rtig.live.sock  — rtig → web (one way)
#   1Hz snapshot of full state
#   Immediate event on each received TLP
#
# Command socket  /tmp/pci.rtig.cmd.sock  — web → rtig (req/ack)
#   PING                   → {"v":1,"t":"pong"}
#   RECEIVE <xml_line>     → {"v":1,"t":"ack","ok":true,"pulsed":[...]}
#   RELOAD_RULES           → {"v":1,"t":"ack","cmd":"RELOAD_RULES","ok":true}
...
log = logging.getLogger('pci.rtig.ipc')
...
class IPCServer:
    def __init__(self):
        ...
        self._receive_cb   = None   # (xml_body: str) → list[str]  pulsed signals
    ...
    def set_receive_callback(self, cb):
        """cb(xml_body) → list of pulsed signal names."""
        self._receive_cb = cb
    ...
    def start(self):
        threading.Thread(target=self._push_accept, daemon=True,
                         name='rtig-push-accept').start()
        threading.Thread(target=self._push_loop,   daemon=True,
                         name='rtig-push-loop').start()
    ...
    def _dispatch(self, line):
        ...
        if cmd == 'RECEIVE':
            if self._receive_cb is None:
                return {'v': 1, 't': 'err', 'msg': 'no receive handler'}
            try:
                pulsed = self._receive_cb(rest)
                return {'v': 1, 't': 'ack', 'ok': True, 'pulsed': pulsed}
            except Exception as e:
                log.warning("receive error: %s", e)
                return {'v': 1, 't': 'ack', 'ok': False, 'error': str(e)}

        if cmd == 'RELOAD_RULES':
            if self._reload_cb:
                self._reload_cb()
            return {'v': 1, 't': 'ack', 'cmd': 'RELOAD_RULES', 'ok': True}
